...
static int _pal_ = 0;
static dac_channel_t _dac_ch = DAC_CHANNEL_1;  // @chrmlinux03
...
static esp_err_t start_dma(int line_width, int samples_per_cc, int ch = 1)
{
  ...
  dac_output_enable(_dac_ch);           			// DAC, video on GPIO25/GPIO26
...
ESP_8_BIT_composite::~ESP_8_BIT_composite()
{
  ...
  if (_started)
  {
    ...
    dac_output_disable(_dac_ch); // @chrmlinux03
    if (!_pal_) {
...
*/
void ESP_8_BIT_composite::begin(dac_channel_t dac_ch)
{
  instance_check();
  _dac_ch = dac_ch;
